...
teleprog = let
        op1 = Unitary $ 
            (C X <.> I) <> (H <.> I <.> I) -- Replace by your own program
        
        op2 = Unitary $ 
            (C (I <.> Z)) <> (I <.> C X) -- Replace by your own program
    in
        [op1, Measure [0,1], op2]
...
teleport rng psi = let
        bell    =  (1 / sqrt 2) .* (ket [0,0] .+ ket [1,1]) -- replace by actual definition, N&C Section 1.3.6
        psi_bell = psi <.> bell
    in
